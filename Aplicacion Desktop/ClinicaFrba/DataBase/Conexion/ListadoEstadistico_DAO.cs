...
/*Para otro form*/
        ...
        {   /*
            MessageBox.Show("SELECT TOP 5 es.descripcion as Especialidad, COUNT(co.id_turno) as Cantidad FROM "
                                                                  + ConstantesBD.tabla_consulta + " as co INNER JOIN "
                                                                  + ConstantesBD.tabla_turno + " t ON co.id_turno = t.id_turno INNER JOIN "
                                                                  + ConstantesBD.tabla_esp_por_profesional + " as esxp ON t.id_profesional = esxp.id_profesional INNER JOIN "
                                                                  + ConstantesBD.tabla_profesional + " as pro ON esxp.id_profesional = pro.id_profesional INNER JOIN "
                                                                  + ConstantesBD.tabla_especialidad + " as es ON esxp.id_especialidad = es.id_especialidad "
                                                                  + " WHERE t.desc_estado = 0 AND MONTH(co.desc_hora_consulta) "
                                                                  + condicionM + "  AND YEAR (co.desc_hora_consulta) " + condicionA
                                                                  + " GROUP BY es.descripcion"
                                                                  + " ORDER BY COUNT(co.id_turno) desc");
            */
            SqlDataReader resultado;
            resultado = this.GD2C2016.ejecutarSentenciaConRetorno("SELECT TOP 5 es.descripcion as Especialidad, COUNT(co.id_turno) as Cantidad FROM "                                                                  + ConstantesBD.tabla_consulta + " as co INNER JOIN "                                                                  + ConstantesBD.tabla_turno + " t ON co.id_turno = t.id_turno INNER JOIN "                                                                  + ConstantesBD.tabla_esp_por_profesional + " as esxp ON t.id_profesional = esxp.id_profesional INNER JOIN "                                                                  + ConstantesBD.tabla_profesional + " as pro ON esxp.id_profesional = pro.id_profesional INNER JOIN "                                                                  + ConstantesBD.tabla_especialidad + " as es ON esxp.id_especialidad = es.id_especialidad "                                                                  + " WHERE t.desc_estado = 0 AND MONTH(co.desc_hora_consulta) "                                                                   + condicionM + "  AND YEAR (co.desc_hora_consulta) " + condicionA                                                                  + " GROUP BY es.descripcion"                                                                  + " ORDER BY COUNT(co.id_turno) desc");
            return resultado;
        }
