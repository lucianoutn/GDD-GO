...
namespace ClinicaFrba.DataBase.Conexion
{
    class RegistroLlegada_DAO : GDD_GO_DAO
    {
        ...
        public List<int> turnosHoy(string profElegido)
        {
            ...
            SqlDataReader lector = this.GD2C2016.ejecutarSentenciaConRetorno("select * from GDD_GO.turno t, GDD_GO.horario h left join GDD_GO.consulta co on co.id_turno=h.id_turno where t.id_turno = h.id_turno and convert(date, h.desc_hora_desde) = convert(date, '" + cambiarFormatoFecha(ConstantesBD.fechaSistema) + "',120) and  t.id_profesional = " + idprofElegido + " and  t.desc_estado = 0 and co.id_turno is null order by desc_hora_desde asc");
        ...
        public int getCantBonosDisponibles(int id_afiliado, bool esFamiliarPpal) /*verifica tambien que los bonos sean del mismo plan actual del afiliado */
        ...
        public int getUnBonoDisponible(int id_afiliado, bool esFamiliarPpal) /*verifica tambien que los bonos sean del mismo plan actual del afiliado */
        {   //estado bono =1: "Disponible" estado =0:"utilizado"
